...
function cmtPost (id) {
	$('.the' + id).find('.cmt-post-form').submit(function () {
		$form = $(this);
...
//	showLikeList(i);
	$('.the' + id + ' .one-cmt.cmt-' + i).find('.like-button#like-cmt').click(function () {
		...
		$.ajax({
			...
			datatype: 'json',
			success: function (data) {
				if (liketext == 'Like') $cmt.find('#like-cmt .text').text('Unlike');
				else if (liketext == 'Unlike') $cmt.find('#like-cmt .text').text('Like');
				$cmt.find('.num-like-cmt').load(url + ' .' + bclas + '.the' + id + ' .one-cmt.cmt-' + i + ' .num-like-cmt > b');
			}
		});
		return false
	})
...
function submitChildCommment (id, i) {
	$('.the' + id + ' .one-cmt.cmt-' + i).find('#cmt' + i + '-' + id).each(function () {
		$cmt = $(this).closest('.one-cmt');
		page = $big.attr('data-p');
		$cmtCmt = $cmt.find('#cmt' + i + '-' + id);
		$(this).find('.sceditor-container iframe').contents().bind("keydown", function (e) {
			if (e.keyCode == 13 && !e.shiftKey) {
				e.preventDefault();
				$cmtCmt.submit();
			}
...
function showChat () {
	if ($('.chat-area').is(':hidden')) {
		...
		left = $('.main-content').position().left - 1;
		$('.right-sidebar').animate({
			'left' : left
		}, 300).css('box-shadow', '-5px 0 5px -5px #eee');
