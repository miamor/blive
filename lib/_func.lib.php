...
function tag ($content) {
	global $u, $frArN;
	$content = str_replace(array('&nbsp;', '@'), array(' ', '+'), _content($content));
...
//		$gdid = getRecord('promise_did', "`iid` = $iid");
	...
	$totalVotes = $gdlBelieve + $gdlBelieveNot + $gdlKnow + $gdlKnowNot ?>
		<div class="right"><a class="view-all-vote">View all votes</a></div>
...
<?	} ?>
	<? if ($gdi['lock'] != 'yes' && $gdi['uid'] == $u) { ?>
		<div class="votes-and-lock gensmall">
