...
<div class='following'>
	...
	<div class='statuss'>
		...
		$mconn = "$mconn AND (`privacy` = 'public' OR (`privacy` = 'draff' AND `uid` = '$u') OR (`privacy` = 'include' AND `available_list` LIKE '%$u%') OR (`privacy` = 'exclude' AND `available_list` NOT LIKE '%$u%') )";

		if ($_GET['note'] == 'undividepage') $l = $getRecord -> GET('activity', $mconn, '', 'time DESC, id DESC');
		else $l = $getRecord -> GET('activity', $mconn, '^5', 'time DESC, id DESC');
